...
public class GameModeLabel2 extends JPanel implements KeyListener {
    ...
    public GameModeLabel2() {
        ...
        mainLabel = new JLabel(new ImageIcon(backgroundIcon.getImage().getScaledInstance(Main.SCREEN_WIDTH[1], Main.SCREEN_HEIGHT[1], Image.SCALE_SMOOTH)));
        mainLabel.setSize(Main.SCREEN_WIDTH[1], Main.SCREEN_HEIGHT[1]);

        // 타이틀 레이블 설정
        JLabel title = new JLabel("Select Game Mode");
        title.setFont(new Font("Arial", Font.BOLD, 40));
        title.setForeground(Color.BLACK);
        title.setBounds(50, Main.SCREEN_HEIGHT[1] / 8, 400, 50);
        mainLabel.add(title);

        // 메뉴 아이템 추가
        int startY = Main.SCREEN_HEIGHT[1] * 5 / 9;
        ...
        keyMessage.setBounds(Main.SCREEN_WIDTH[1] / 2 - 300, Main.SCREEN_HEIGHT[0] / 2 - 100, 600, 100);
        add(keyMessage);
    ...
    private void activateMenuItem(int index) {
        switch (index) {
            case 0: // Classic Mode
                ...
                switchToScreen(Main.classicMode2);
                // Classic Mode에 대한 동작 추가
                break;
            case 1: // Item Mode
                System.out.println("Item Mode Selected");
                switchToScreen(Main.itemMode2);
                // Item Mode에 대한 동작 추가
                break;
            case 2: // Back
